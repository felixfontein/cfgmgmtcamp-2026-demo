...
- name: Convert IP address to DNS name for reverse lookup
  ansible.builtin.set_fact:
    dns_name: "{{ ip_address | felixfontein.antsibull_nox_playground.reverse_pointer }}"
...
def reverse_pointer(ip: t.Any) -> str:
    assert_requirements_present("felixfontein.antsibull_nox_playground.reverse_pointer", "filter")
    if not isinstance(ip, (str, bytes)):
        raise AnsibleFilterError(
            "Input for felixfontein.antsibull_nox_playground.reverse_pointer must be a string"
        )
    try:
