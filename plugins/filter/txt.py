...
def quote_txt(
    ...
    if not isinstance(value, (str, bytes)):
        raise AnsibleFilterError("Input for felixfontein.antsibull_nox_playground.quote_txt must be a string")
...
def unquote_txt(value: t.Any, character_encoding: t.Any = "decimal") -> str:
    if not isinstance(value, (str, bytes)):
        raise AnsibleFilterError("Input for felixfontein.antsibull_nox_playground.unquote_txt must be a string")
    if character_encoding not in ("decimal", "octal"):
        raise AnsibleFilterError(
            f'character_encoding must be "decimal" or "octal", not {character_encoding!r}'
        )
