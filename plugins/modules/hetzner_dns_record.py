...
description:
  ...
  - If you do not want to add/remove values, but replace values, you will be interested in modifying a B(record set) and not
    ...
    M(felixfontein.antsibull_nox_playground.hetzner_dns_record_set) module for working with record sets.
...
- name: Add a new.foo.com A record
  felixfontein.antsibull_nox_playground.hetzner_dns_record:
    state: present
    zone: foo.com
    record: new.foo.com
    type: A
