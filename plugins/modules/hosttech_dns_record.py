...
description:
  ...
  - If you do not want to add/remove values, but replace values, you will be interested in modifying a B(record set) and not
    a single record. This is in particular important when working with C(CNAME) and C(SOA) records. Use the
    M(felixfontein.antsibull_nox_playground.hosttech_dns_record_set) module for working with record sets.
extends_documentation_fragment:
  ...
  - felixfontein.antsibull_nox_playground.module_record
...
- name: Add a new.foo.com A record
  felixfontein.antsibull_nox_playground.hosttech_dns_record:
    state: present
    zone: foo.com
    record: new.foo.com
    type: A
...
from ansible_collections.felixfontein.antsibull_nox_playground.plugins.module_utils.module.record import (
