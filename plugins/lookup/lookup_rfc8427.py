...
- name: Look up A (IPv4) records for example.org in RFC 8427 JSON format
  ansible.builtin.debug:
    msg: "{{ query('felixfontein.antsibull_nox_playground.lookup_rfc8427', 'example.org.') }}"

- name: Look up AAAA (IPv6) records for example.org in RFC 8427 JSON format
  ansible.builtin.debug:
    msg: "{{ query('felixfontein.antsibull_nox_playground.lookup_rfc8427', 'example.org.', type='AAAA' ) }}"

- name: Get complete DNS message for MX records
  ansible.builtin.debug:
    msg: "{{ query('felixfontein.antsibull_nox_playground.lookup_rfc8427', 'example.org.', type='MX' ) }}"
"""
...
class LookupModule(LookupBase):
    ...
    def run(
        ...
        assert_requirements_present_dnspython("felixfontein.antsibull_nox_playground.lookup_rfc8427", "lookup")
        ...
        if self.get_option("server"):
            ...
            assert_requirements_present_ipaddress(
                "felixfontein.antsibull_nox_playground.lookup_rfc8427", "lookup"
            )
