...
options:
  ...
  type:
    description:
      ...
      - Support for V(HTTPS) and V(SVCB) has been added in felixfontein.antsibull_nox_playground 3.4.0.
    type: str
    default: A
    choices:
      - A
  ...
  search:
    description:
      ...
      - If V(true), the input is assumed to be a relative domain name if it does not end with C(.), the search list configured
        ...
        relative names.
      - Note that this behavior changed in felixfontein.antsibull_nox_playground 3.0.0. In felixfontein.antsibull_nox_playground
        2.x.y, O(search=false) was the only available choice.
    type: bool
    default: true
    version_added: 3.0.0
notes:
...
- name: Look up A (IPv4) records for example.org as a list of dictionaries
  ansible.builtin.debug:
    msg: "{{ query('felixfontein.antsibull_nox_playground.lookup_as_dict', 'example.org.') }}"

- name: Look up AAAA (IPv6) records for example.org as a list of IPv6 addresses
  ansible.builtin.debug:
    msg: "{{ query('felixfontein.antsibull_nox_playground.lookup_as_dict', 'example.org.', type='AAAA' ) | map(attribute='address') }}"

- name: Look up TXT records for ansible.com as a list of strings
  ansible.builtin.debug:
    msg: "{{ query('felixfontein.antsibull_nox_playground.lookup_as_dict', 'ansible.com.', type='TXT' ) | map(attribute='value') }}"
"""
...
from ansible_collections.felixfontein.antsibull_nox_playground.plugins.module_utils.dnspython_records import (
    NAME_TO_RDTYPE,
    NAME_TO_REQUIRED_VERSION,
    convert_rdata_to_dict,
)
from ansible_collections.felixfontein.antsibull_nox_playground.plugins.module_utils.ips import is_ip_address
from ansible_collections.felixfontein.antsibull_nox_playground.plugins.module_utils.resolver import (
    SimpleResolver,
)
from ansible_collections.felixfontein.antsibull_nox_playground.plugins.plugin_utils.ips import (
    assert_requirements_present as assert_requirements_present_ipaddress,
)
from ansible_collections.felixfontein.antsibull_nox_playground.plugins.plugin_utils.resolver import (
...
class LookupModule(LookupBase):
    ...
    def run(
        ...
        assert_requirements_present_dnspython("felixfontein.antsibull_nox_playground.lookup", "lookup_as_dict")
        ...
        if self.get_option("server"):
            ...
            assert_requirements_present_ipaddress(
                "felixfontein.antsibull_nox_playground.lookup", "lookup_as_dict"
            )
